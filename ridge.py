...
clf = LogisticRegressionCV(
    cv=5,
    penalty='l2',          # 改为L2正则化（岭回归）
    solver='lbfgs',        # 选择支持L2的优化器（如lbfgs/newton-cg/sag）
    max_iter=10000,        # 保持足够迭代次数
    scoring='roc_auc_ovr', # 保持AUC评估
    multi_class='ovr',     # 多分类策略不变
    Cs=10                  # 正则化强度的倒数范围（默认10个值，可自定义）
)
...
if y_test_bin.shape[1] == 1:
    ...
    plt.title('Ridge Regression ROC Curve with 95% CIs')
