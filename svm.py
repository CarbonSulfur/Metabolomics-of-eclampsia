...
# 定义参数网格（示例值，可根据需求调整）
param_grid = {
    'C': [0.001, 0.01, 0.1, 1, 10, 100],  # SVM的正则化参数
    'kernel': ['linear', 'rbf'],           # 线性或径向基核函数
    'gamma': ['scale', 'auto']             # 仅对rbf核有效
}

# 创建SVM分类器，启用概率估计以支持AUC计算
svm = SVC(
    probability=True,               # 必须启用以计算ROC AUC
    decision_function_shape='ovr',  # 多分类策略与LogisticRegression一致
...
clf = GridSearchCV(
    ...
    cv=5,                          # 5折交叉验证
    scoring='roc_auc_ovr',         # 评分指标与原来一致
    refit=True,                    # 找到最佳参数后重新训练完整数据集
...
if y_test_bin.shape[1] == 1:
    ...
    plt.title('SVM Regression ROC Curve with 95% CIs')
